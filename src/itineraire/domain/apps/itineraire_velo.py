...
def itineraire_parking_velo(
        departure_address: str, arrival_address: str, to_parking: bool, parking_filter: ParkingVeloFilters = ParkingVeloFilters.default) -> dict:
    api_handler = ApiHandler()
    itinerary = calcul_itineraire_velo(
        api_handler, departure_address, arrival_address, to_parking, parking_filter)
    return itinerary


if __name__ == "__main__":
    itinerary = itineraire_parking_velo(
        "34 avenue de l'opera", "62 Rue Jean-Jacques Rousseau", True, ParkingVeloFilters.casier
    )
    print(itinerary)
